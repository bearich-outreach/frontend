...
- Name: e2e.spec.ts >> Cashflow App >> cashflow sidebar active state
...
Locator: locator('a.nav-link-active').filter({ hasText: 'Transaksi' })
Expected: visible
Error: strict mode violation: locator('a.nav-link-active').filter({ hasText: 'Transaksi' }) resolved to 2 elements:
    1) <a class="nav-link-active" href="/apps/cashflow/transactions">…</a> aka getByRole('link', { name: 'Transaksi', exact: true })
    2) <a class="nav-link-active shrink-0" href="/apps/cashflow/transactions">Transaksi</a> aka locator('header').getByText('Transaksi', { exact: true })

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for locator('a.nav-link-active').filter({ hasText: 'Transaksi' })
